...
# VERSION:      1.0.22
# DESCRIPTION:  Scout installation script for multiple OS/Distributions
# BUGS:         support@scoutapp.com
# COPYRIGHT:    (c) 2015 Scout
# LICENSE:      Apache 2.0; http://www.apache.org/licenses/LICENSE-2.0
# ORGANIZATION: http://scoutapp.com
#
...
check_distro_supported() {
    ...
    case "${DISTRO_NAME_L}" in
        "ubuntu")
            if [ ${DISTRO_MAJOR_VERSION} -lt 10 ] || [ ${DISTRO_MAJOR_VERSION} -gt 16 ]; then
                distro_not_supported
            fi
            ;;
        "debian")
            if [ ${DISTRO_MAJOR_VERSION} -lt 7 ] || [ ${DISTRO_MAJOR_VERSION} -gt 8 ]; then
                distro_not_supported
            fi
            ;;
        red_hat*|"centos")
            ...
            ;;
        "amazon_linux_ami")
            if [ ${DISTRO_MAJOR_VERSION} -gt 2016 ]; then
                distro_not_supported
            fi
            ;;
        "oracle_linux")
...
usage() {
  ...
  Script Options:
    ...
    --no-cron-detect   Do not try to detect scout information from cron files
    --no-cron-disable  Do not disable detected scout cron entries
    --reinstall        Reinstall scoutd
    -y, --yes          Assume yes to all questions (non-interactive).

  Scoutd Options:
    -e, --environment ENVIRONMENT    Environment for this server. Environments are defined through the scoutapp.com web UI
    -r, --roles role1,role2          Roles for this server. Roles are defined through the scoutapp.com web UI
...
_configure_repo() {
    ...
    case "${DISTRO_NAME_L}" in
        "ubuntu")
            ...
            if [ ${DISTRO_MAJOR_VERSION} -eq 16 ]; then
...
_restart_scoutd() {
    ...
    case "${DISTRO_NAME_L}" in
        ...
        red_hat*|"centos"|"oracle_linux"|"scientific_linux")
            ...
            ;;
        "amazon_linux_ami")
            if [ ${DISTRO_MAJOR_VERSION} -lt 2017 ]; then
                if ! ( /sbin/initctl stop scout >/dev/null 2>&1 && /sbin/initctl start scout >/dev/null 2>&1 ); then
                    /sbin/initctl start scout
                fi
            else
...
_contact_us_for_help() {
    echo -e "Please contact us at ${BC}support@scoutapp.com${EC} if you need assistance."
}
...
do_scoutd_reinstall() {
    ...
    echowarn "Refer to the documentation for configuring scoutd: http://help.scoutapp.com/docs/agent"
    exit 0
}
...
_install_ping() {
    if [ "$_SD_ALREADY_PINGED" != "$__IS_TRUE" ]; then
        if $(_is_account_key "$1"); then
            ...
            _ping_str="https://scoutapp.com/install_ping/${1}/?OS_NAME_L=${OS_NAME_L}&OS_VERSION_L=${OS_VERSION_L}&DISTRO_NAME_L=${DISTRO_NAME_L}&DISTRO_VERSION=${DISTRO_VERSION}&RUBY_PATH=${__install_ping_ruby_path}"
...
echoyellow "    http://help.scoutapp.com/docs/agent"
echoblue ""
echoblue "  If you have any questions or problems, please contact us:"
echoyellow "    email: support@scoutapp.com"
echoblue " ==============================================================="
echo     ""
